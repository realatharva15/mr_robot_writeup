...
```
we find eliot's wordpress password! lets use this to login. and just like that we have a dashboard as elliot. now lets navigate to the appearance tab and then to the editor section. now select the 404 template. replace the contents with a php reverse shell. now once we have uploaded we will start a netcat listener.
...
```
surprisingly the password which gets cracked is in all caps. also it is the last word from the wordlist. lets try all lowercase as the password. and it worked! lets quickly read and submit the key2. 
